...
def test_complete_execution_emits_manifest():
    ...
    assert result.run.result.status == "ready_for_snapshot"
...
def test_failed_cell_emits_no_manifest():
    ...
    assert result.run.result.status == "blocked_incomplete_coverage"
    assert result.snapshot_manifest is None
    assert ("CHE", 2017, "credit_gdp") in result.run.result.missing
